...
export function resolveRaisedElevationStyle({
  pressed,
  disabled,
  bottomBorderWidth,
  offset,
  bottomBorderColor,
}: RaisedElevationInput): RaisedElevationStyle {
  const sinking = pressed && !disabled
  return {
    // 押下前後で不変。ここを変えるとレイアウトが動く。
    borderBottomWidth: bottomBorderWidth,
    borderBottomColor: sinking ? "transparent" : bottomBorderColor,
    transform: [{ translateY: sinking ? offset : 0 }],
  }
}
