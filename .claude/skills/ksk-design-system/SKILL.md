...
description: KSK Design System で UI を実装・修正・レビューするとき必ず適用する判断ルール。トークン設計（semantic のみ・生値禁止）・禁止パターン（正本: rules.json）・HIG/M3 基準・コンポーネント選択の判断基準を定義する。React + Tailwind v4 + TypeScript の UI コードを書く前に読むこと。
---
...
1. `src/components/COMPONENT_LOOKUP.md` — 既存コンポーネントの索引。**UI を書く前に必ず確認**
2. `contracts/rules.json` — 禁止パターン＋AIアンチパターンの正規表現つき正本
3. `tokens.json` / `src/preset.css` — トークン値の正本
4. `contracts/components.json` — variant・使用ルールの正本
5. 画面（ページ/ダイアログ等）を実装する場合は `contracts/screen-patterns.json`（どのシェル/パターンを使うか、decisionTree）と `contracts/composition.json`（内部の並べ方）
...
## 3. 禁止パターン Top10（全件は rules.json、rationale は references/）
...
| 状況 | 読む |
|---|---|
| 禁止パターンに引っかかった・例外を検討したい | `references/prohibited-rationale.md`（全件の why） |
