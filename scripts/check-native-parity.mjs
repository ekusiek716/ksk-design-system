...
const INTENTIONAL_PROP_GAPS = new Map([
  ...
  ["CoachMarkOverlay", { webOnly: ["steps", "onComplete", "onSkip", "variant", "ringColor", "maxWidth", "labels"], nativeOnly: ["onClose", "highlight", "children"], reason: R_INDEPENDENT + " (Web: querySelector-driven DOM spotlight tour; Native has its own targeting model). labels (issue #477) i18n's the tour's own next/done/skip buttons and aria-label — Native's overlay renders no buttons of its own (it only dims around a highlight and hosts children), so the labels live on the Native CoachMark (nextLabel/skipLabel) instead" }],
  ...
  ["ProgressRing", { webOnly: ["label", "className", "strokeWidth", "color", "trackColor"], nativeOnly: ["max", "thickness"], reason: R_STYLE + "; max is inherited via extends on Web. strokeWidth (issue #480) is Web's name for the same concept Native already exposes as `thickness` — the two are a naming drift that predates #480; renaming Native's public prop would break existing native consumers for no visual gain, so they stay listed on both sides. color/trackColor (issue #480) take semantic CSS `var(--...)` strings, which React Native cannot resolve — Native reads the same roles from ThemeProvider (`theme.brand.primary` / `theme.surface.tertiary`), so a var()-string prop has no meaning there" }],
