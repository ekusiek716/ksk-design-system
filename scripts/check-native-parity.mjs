...
const INTENTIONAL_PROP_GAPS = new Map([
  ...
  ["Label", { webOnly: ["unstyled"], nativeOnly: ["required"], reason: `unstyled (issue #443, follow-up to #420) strips the Web Tailwind layer (typo-label-md / text color / peer-disabled classes) for gradual migration — Native has no CSS layer to strip (styles are StyleSheet/inline). Native's required renders an inline "*" marker Text node not yet ported to Web (Web consumers compose the marker as a child span instead, see label.stories.tsx WithRequiredMarker).` }],
