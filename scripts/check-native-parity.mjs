...
const INTENTIONAL_NATIVE_GAPS = new Map([
  ...
  ["FilterPill", "任意コンテンツの Popover（Radix Popover + DOM アンカー配置）に依存する Web 専用のフィルタピル。native の同等 UX は BottomSheetFrame + Chip で、アンカー付き popover ではなくボトムシートで開く"],
  ["ImageOverlayAction", "画像上のオーバーレイ操作。native には同役割の MediaActionCluster（44pt ヒットエリア + glass 面）が既にあり、こちらは backdrop-blur / absolute 配置という Web の重ね方に特化しているため移植しない"],
  ["ThumbnailCard", "カード全体クリックを stretched link（絶対配置の不可視 a/button を重ねる）で実現する Web 専用パターン。native は Pressable でカードごと押せるため、この構造自体が不要"],
  ...
  ["ResponsiveOverlayFrame", "viewport 幅でボトムシート↔中央モーダルを切り替える Web 専用 frame（issue #472）。matchMedia と CSS のブレークポイントに依存し、native は画面幅に関わらずボトムシート（BottomSheetFrame）で統一するため移植しない"],
...
const INTENTIONAL_PROP_GAPS = new Map([
  ...
  ["CoachMark", { webOnly: ["content", "children", "placement", "variant", "open", "onOpenChange", "totalSteps", "showClose", "onClose", "delayDuration", "className", "ariaLabel"], nativeOnly: ["title", "description", "total", "onSkip"], reason: R_INDEPENDENT + " (Web: Radix Popover-anchored tooltip with content slot; Native: title+description fields with its own overlay). ariaLabel (issue #477) overrides the aria-label Radix renders in its visually-hidden role=\"tooltip\" copy of the balloon — a Radix Tooltip implementation detail with no Native counterpart (RN reads the Text nodes directly)" }],
  ["CoachMarkOverlay", { webOnly: ["steps", "onComplete", "onSkip", "variant", "ringColor", "maxWidth", "labels", "autoFocus", "restoreFocusOnClose", "closeOnEsc"], nativeOnly: ["onClose", "highlight", "children"], reason: R_INDEPENDENT + " (Web: querySelector-driven DOM spotlight tour; Native has its own targeting model). autoFocus/restoreFocusOnClose/closeOnEsc (issue #504) are the DOM focus-trap controls: RN has no Tab traversal or document.activeElement to trap, and its overlay renders no controls of its own. labels (issue #477) i18n's the tour's own next/done/skip buttons and aria-label — Native's overlay renders no buttons of its own (it only dims around a highlight and hosts children), so the labels live on the Native CoachMark (nextLabel/skipLabel) instead" }],
  ...
  ["DateField", { webOnly: ["className", "dateFormat", "triggerLabel"], nativeOnly: ["formatter", "accessibilityLabel"], reason: "dateFormat (format string) vs formatter (function) diverged independently for the same feature; accessibilityLabel/triggerLabel (issue #426) are the same trigger label in platform-native vocabulary" }],
  ["DatePicker", { webOnly: ["id", "className", "dateFormat", "triggerLabel", "defaultMonth", "min", "max"], nativeOnly: ["minDate", "maxDate", "formatter", "accessibilityLabel"], reason: "min/max vs minDate/maxDate naming and dateFormat-string vs formatter-function diverged independently; accessibilityLabel/triggerLabel (issue #426) are the same trigger label in platform-native vocabulary" }],
  ...
  ["Input", { webOnly: ["startAdornment", "endAdornment", "showCount", "unstyled"], nativeOnly: ["invalid", "disabled", "leading", "trailing", "style", "inputStyle"], reason: "startAdornment/endAdornment (Web) vs leading/trailing (Native) naming diverged independently for the same adornment-slot feature; invalid/disabled are inherited via extends on Web (invisible to this extractor); unstyled (issue #420) strips the Web Tailwind layer for gradual migration — Native has no CSS layer to strip (styles are StyleSheet/inline). style/inputStyle (issue #447) are RN's className-equivalent styling escape hatches (container vs inner TextInput) — Web achieves the same with `className`, which is intentionally Web-only here (see startAdornment/endAdornment note, className itself was already omitted from this diff historically as R_STYLE)." }],
  ["KeyboardAwareSheetFooter", { webOnly: [], nativeOnly: ["children", "style"], reason: R_STYLE }],
  ["Label", { webOnly: ["unstyled"], nativeOnly: ["required", "children"], reason: `unstyled (issue #443, follow-up to #420) strips the Web Tailwind layer (typo-label-md / text color / peer-disabled classes) for gradual migration — Native has no CSS layer to strip (styles are StyleSheet/inline). Native's required renders an inline "*" marker Text node not yet ported to Web (Web consumers compose the marker as a child span instead, see label.stories.tsx WithRequiredMarker).` }],
  ["ListItem", { webOnly: [], nativeOnly: ["leading", "title", "description", "trailing", "footerSlot", "showChevron", "onPress", "disabled", "align", "density", "titleTone", "descriptionTone", "divider"], reason: "Native titleTone/descriptionTone preserve string typography; divider owns density-dependent insets (#572/#573). Web uses slot styling and border classes. Web ListItem's props live in a discriminated union (ListItemLinkProps | ListItemButtonProps | ListItemStaticProps) built from a separate ListItemCommonProps interface, so this `${Name}Props`-interface-body extractor sees no own props on the Web side and reports every Native prop as Native-only. footerSlot/align/density (issue #355) exist on BOTH platforms with the same vocabulary and semantics — an extraction artifact, not a real gap. leading/trailing are Native's naming for Web's leftSlot/rightSlot (independent naming drift predating this check); showChevron/onPress/disabled are likewise declared directly on Native and inside the union on Web" }],
  ...
  ["MultiSelect", { webOnly: ["value", "emptyLabel", "className", "maxDisplay", "clearable", "clearLabel", "triggerLabel"], nativeOnly: ["values", "accessibilityLabel"], reason: "value vs values naming diverged independently; Web has a richer display-truncation/clearable API not yet ported to Native. clearLabel (issue #428) labels the Web-only clear button; triggerLabel vs accessibilityLabel (issue #426) are the same trigger label in platform vocabulary" }],
  ...
  ["ResponsiveDialog", { webOnly: ["children"], nativeOnly: ["breakpoint"], reason: R_INTERACTION + "; breakpoint は issue #472 で Web にも入った（own prop ではなく ResponsiveOverlayBreakpointOptions 経由なのでこの抽出には出ない）が型が異なる（Web = \"sm\"|\"md\"|\"lg\"|\"xl\"|\"product-theme\" のトークンで CSS メディアクエリに解決 / Native = number px を Dimensions と比較）。Web は加えて breakpointQuery で生のメディアクエリを受ける" }],
  ...
  ["StatCard", { webOnly: ["unit", "icon", "variant", "interactive", "onClick"], nativeOnly: ["delta", "style", "labelMinHeight", "onLabelLayout"], reason: "Native uses StyleProp<ViewStyle> for stretch (Web inherits div style/className); labelMinHeight/onLabelLayout expose RN natural text measurement for Dynamic Type, while Web uses CSS layout. " + R_VARIANT_ARTIFACT + "; Web adds a unit/interactive-click API and Native adds a delta (trend) display, neither ported to the other side yet" }],
