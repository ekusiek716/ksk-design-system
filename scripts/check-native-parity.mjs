...
const INTENTIONAL_PROP_GAPS = new Map([
  ...
  ["CoachMark", { webOnly: ["content", "children", "placement", "variant", "open", "onOpenChange", "totalSteps", "showClose", "onClose", "delayDuration", "className", "ariaLabel", "contentRef"], nativeOnly: ["title", "description", "total", "onSkip"], reason: R_INDEPENDENT + " (Web: Radix Popover-anchored tooltip with content slot; Native: title+description fields with its own overlay). contentRef (issue #504) hands out the DOM node of the Portal-rendered balloon so a focus trap can include it — Native has no Portal/DOM to hand out. ariaLabel (issue #477) overrides the aria-label Radix renders in its visually-hidden role=\"tooltip\" copy of the balloon — a Radix Tooltip implementation detail with no Native counterpart (RN reads the Text nodes directly)" }],
