...
const INTENTIONAL_PROP_GAPS = new Map([
  ...
  ["SwipeRow", { webOnly: ["actions", "side", "className"], nativeOnly: ["rightActions", "actionWidth", "accessibilityActions", "onAccessibilityAction"], reason: `actions vs rightActions naming diverged independently for the same swipe-action-list feature. accessibilityActions/onAccessibilityAction (issue #342) are RN's accessibility-actions API (VoiceOver rotor / TalkBack custom actions) with no DOM equivalent; Web's parity fix instead makes the always-rendered action \`<button>\`s focus-reveal via onFocus/onBlur on the actions group (native-parity.mjs can't see this since it isn't a props-interface change)` }],
