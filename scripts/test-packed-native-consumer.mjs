...
try {
  ...
  writeFileSync(
    ...
    [
      'import { Button, ListItem, Radio, ThemeProvider, type ListItemDivider, type ListItemTone, type RadioProps } from "ksk-design-system/native/ui"',
      ...
      "void defaultTheme",
      "",
      'const tone: ListItemTone = "accent"',
      'const divider: ListItemDivider = "inset"',
      'const radioProps: RadioProps = { selected: true, onChange: (selected: boolean) => { void selected }, accessibilityLabel: "選択肢" }',
      "",
      "export function Consumer() {",
      '  return <ThemeProvider initialName="default">',
      '    <Button>確認する</Button>',
      '    <Radio {...radioProps} />',
      '    <Radio selected decorative size={20} disabled />',
      '    <ListItem title="出典" description="参考資料" titleTone={tone} descriptionTone="caution" divider={divider} density="compact" />',
      '  </ThemeProvider>',
      "}",
      "",
    ].join("\n"),
