...
- 判断基準（何が禁止か・なぜか）の正本は `contracts/rules.json` と `.Codex/skills/ksk-design-system/` — 本スキルはそれを使った**巡回手順**のみを定義する。独自の禁止パターン判定を作らない。
...
各ストーリーを Codex のブラウザツール群（resize_window / navigate / computer screenshot 等）で **4パターン**確認する:
