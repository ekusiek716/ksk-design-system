...
KSK のセルフチェック（AGENTS.md 冒頭）を必ず守る:
...
## 既知の落とし穴（フィードバック由来・再発防止）

過去のレビュー指摘を根本原因まで遡って恒久ルール化したもの。生成時に必ず織り込む。

追記は「都度」ではなく **値する時だけ**: ①再発しうる体系的な原因（DS の挙動・構造の癖）で、
②生成時に自動で防げる ものに限る。一回限りのミス・好み/レイアウトの微調整・ホスト側の
問題は、ここに足さずその場修正で済ませる。項目が増えて重複してきたら統合・剪定して短く保つ
（長いリストは守られない）。**根本原因が DS 側のバグ/ギャップなら、ここに回避策を書くより
DS の改善イシューを起票して源流を直す**（ここの項目は DS が直るまでの暫定メモ）。

- **DS コンポーネントのデフォルト props を実装で確認してから使う（憶測で使わない）。**
  特に `ChipSelector` は `multiple` のデフォルトが **true**（複数選択）。サイズ等の単一選択には
  必ず `multiple={false}` を渡す（DS 改善は issue #39）。制御/非制御・`defaultValue` の有無・
  `onChange` が返す型も `.tsx` を見て確認する。
- **レスポンシブを既定にする。** ホストのプレビュー枠が `@container` なので、ビューポートの
  `lg:` ではなく **`@container` クエリ（`@md:` / `@xl:` 等）** で組む。SP/PC フレーム切替で実際に
  レイアウトが変わるようにする（SP 専用で作らない）。商品/詳細系の PC 定番は **2 列・画像左**。
  ※ DS シェル（AdminShell 等）は viewport `lg:` + `h-screen` 依存で枠に追従しない（issue #40）。
  枠付きプレビューでは中身に実 DS を使いつつ、シェル骨格は `@container` で内製する。
- **モバイル専用の装飾は PC で隠す/置き換える。** 写真に重なるフローティングナビ・画面下部の
  固定バー・スワイプ前提 UI は PC で浮くので `@xl:hidden` にし、PC では右カラム等へ操作を移す。
- **生成後はレンダリングだけでなくインタラクションも確認する。** 主要操作（選択・数量・トグル）を
  preview で実クリックし、state と表示（価格連動等）が変わることを確かめる（描画 OK＝動作 OK ではない）。
- **選択・編集できる行リスト/テーブルは行を `React.memo` で隔離する。** `DataTableActionCell` 等で
  各行が Radix の `DropdownMenu` を持つと、1 行チェックしただけで親が再レンダーして全行＝全 Radix
  メニューが再生成され「テーブル全体がカクつく」。行コンポーネントを `memo` 化し、トグル系
  コールバックを `useCallback`・行アクション配列を不変参照にして、`selected`(boolean) が変わった
  行だけ再レンダーさせる。
